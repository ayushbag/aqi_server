...
function getIndiaAQI(pollutant: "pm25" | "pm10", concentration: number) {
  const breakpoints = INDIA_BREAKPOINTS[pollutant];

  const range = breakpoints.find(
    (bp) => concentration >= bp.conc_lo && concentration <= bp.conc_hi
  );
...
// FINAL FUNCTION
export const calcualteAQIIndex = async ({
  co,
  no2,
  so2,
  o3,
